...
;Variables
  ...
  !define SETUP_FILE_NAME "CryptnoxGallery-setup"
  !define PRODUCT_VERSION "1.0.0"
  !define PRODUCT_WEB_SITE "https://cryptnox.ch"
  !define PRODUCT_PUBLISHER "Cryptnox SA"
  !define PRODUCT_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\cryptnoxgallery"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\cryptnox-gallery.exe" "" "$INSTDIR\cryptnox.ico"

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\cryptnox-gallery.exe" "" "$INSTDIR\cryptnox.ico" 0
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\cryptnox.ico"
